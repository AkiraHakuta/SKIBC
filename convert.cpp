    ...
    */
    ...
    if (nodeseq[0]->data == "S"){                
        *changed_ptr = true;        
        ...
        node->left->left = nodeseq[1];  
        node->left->right = nodeseq[3]; 
        node->right->left = nodeseq[2];
        node->right->right = nodeseq[3];
        return;
    }
    ...
    if (nodeseq[0]->data == "S'"){                
        *changed_ptr = true;
        node->right = node->left;
        node->left = node->right->left->left;          
        node->left->left = nodeseq[1];
        node->left->right = node->right->left;        
        node->right->left = nodeseq[3];
        node->right->right = nodeseq[4]; 
        node->left->right->left = nodeseq[2];
        node->left->right->right = nodeseq[4];  
        return;
    }
    else if (nodeseq[0]->data == "B'"){                
        *changed_ptr = true;
