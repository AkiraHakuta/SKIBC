...
int main(int argc, const char *args[])
{
    ...
    for (int i = 0; i <  e->comm.size(); i++){
        if (trace){
            step = 0;  
            cout << endl;   
            cout << setw(4) << step << ": "<< e->comm[i][Sb] <<  e->comm[i][Eq] << e->comm[i][TermText] << ";" << endl;  
        }
        commType = stoi(e->comm[i][CommType]);
        if (commType != SbCCEqTerm && commType != NewLine){
            if(commType != SbCEqTerm)
                 e->not_found_sb_eval = true;
            else
                 e->not_found_sb_eval = false;
            result_str =  eval((e->comm[i])[TermText]);
        }   
        ...
        switch (commType){
            case Term:  
                ...
                break;
            case SbEqTerm:
                e->sq_term_map[(e->comm[i])[Sb]]= result_str;
                result_str = (e->comm[i])[Sb] + " = " + result_str + ";";
                break;    
             case SbCEqTerm:
                e->sq_term_map[(e->comm[i])[Sb]]= result_str;
                result_str = e->comm[i][Sb] + " := " + e->comm[i][TermText] + ";";
                break;
            case SbCCEqTerm:
                e->sq_term_map[(e->comm[i])[Sb]]= e->comm[i][TermText];
                result_str = e->comm[i][Sb] + " ::= " + e->comm[i][TermText] + ";";
                break;
            case PutStr:
                break;
